...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for(int x = min_x; x <= max_x; x ++) {
        for(int y = min_y; y <= max_y; y++) {
            ...
            float z_interpolate = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
            z_interpolate *= w_reciprocal;

            int point_index = get_index(x, y);
            if(z_interpolate < depth_buf[point_index]) {
                depth_buf[point_index] = z_interpolate;
